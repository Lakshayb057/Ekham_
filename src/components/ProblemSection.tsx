    ...
    <section id="problem" className="min-h-screen lg:h-screen flex flex-col justify-center py-6 sm:py-8 lg:py-8 bg-[#FAF8F2] relative overflow-hidden border-b border-[#EAE2D7]/80">
      <div className="max-w-[1260px] w-full mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col justify-center h-full">
        ...
        <div id="why-ekhum">
          ...
          {/* Main Comparison Layout: Two Clean Panels with Center Indicator */}
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1.15fr] items-center gap-3 sm:gap-4 mb-4 sm:mb-5">
            
            {/* ---------------- LEFT PANEL: TRADITIONAL OPERATIONS ---------------- */}
            <div className="bg-[#F1EEE7] border border-[#E2DAD0] rounded-2xl sm:rounded-3xl p-4 sm:p-4.5 lg:p-5 relative flex flex-col justify-center h-full min-h-[170px] sm:min-h-[185px]">
              <div>
                <div className="mb-3.5 flex justify-between items-start gap-2">
                  <div>
                    <h3 className="text-xs sm:text-sm font-bold text-[#102126] tracking-tight mb-0.5">
                      Traditional Charity Operations
                    </h3>
                    <p className="text-[11px] sm:text-xs text-[#687176] font-medium">
                      Disconnected tools. Manual work. Limited visibility.
                    </p>
                  </div>
                  <div className="text-[#F4512A] font-black text-[10px] sm:text-xs uppercase tracking-widest text-right shrink-0 mt-1">
                    Many Tools
                  </div>
                </div>

                {/* 5 Horizontal Nodes */}
                <div className="flex items-center justify-between gap-1 overflow-x-auto sm:overflow-visible pb-1 sm:pb-0">
                  {traditionalNodes.map((node, idx) => {
                    const Icon = node.icon;
                    return (
                      <React.Fragment key={idx}>
                        <div className="flex flex-col items-center flex-shrink-0">
                          <div className="w-[54px] sm:w-[60px] lg:w-[58px] xl:w-[64px] h-[56px] sm:h-[62px] lg:h-[60px] xl:h-[66px] bg-white rounded-xl border border-[#E0D8CC] shadow-2xs flex flex-col items-center justify-center p-1 text-center">
                            <Icon className="w-4 h-4 text-[#55605A] stroke-[1.8] mb-0.5" />
                            <span className="text-[8px] sm:text-[9px] font-bold text-[#102126] leading-tight text-center whitespace-pre-line">
                              {node.label}
                            </span>
                          </div>

                          {/* Minus or Cross badge below node */}
                        ...
                        {idx < traditionalNodes.length - 1 && (
                          <div className="flex items-center justify-center text-[#B0A79C] select-none px-0.5 sm:px-1 flex-shrink-0">
                            <span className="text-[9.5px] sm:text-[10px] font-mono tracking-tighter opacity-80">- -›</span>
                          </div>
                        )}
                      </React.Fragment>
                    );
            ...
            <div className="flex lg:flex-col items-center justify-center py-1 lg:py-0 px-2 flex-shrink-0 self-center">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#F4512A] text-white flex items-center justify-center shadow-xs">
                <ArrowRight className="w-4 h-4 sm:w-4.5 sm:h-4.5 stroke-[3]" />
              </div>
            </div>

            {/* ---------------- RIGHT PANEL: WITH EKHUM ---------------- */}
            <div className="bg-[#153D2B] border border-[#1F543C] rounded-2xl sm:rounded-3xl p-4 sm:p-4.5 lg:p-5 relative shadow-md flex flex-col justify-center h-full min-h-[170px] sm:min-h-[185px] overflow-hidden">
              <div className="absolute top-0 right-1/4 w-48 h-12 bg-[#F4512A]/15 blur-xl pointer-events-none rounded-full"></div>

              <div className="relative z-10">
                <div className="mb-3.5 flex justify-between items-start gap-2">
                  <div>
                    <h3 className="text-xs sm:text-sm font-bold text-white tracking-tight mb-0.5">
                      With EKhum
                    </h3>
                    <p className="text-[11px] sm:text-xs text-emerald-100/85 font-normal">
                ...
                {/* 6 Horizontal Nodes */}
                <div className="flex items-center justify-between gap-1 overflow-x-auto sm:overflow-visible pb-1 sm:pb-0 relative z-10">
                  {ekhumNodes.map((node, idx) => {
                    const Icon = node.icon;
                    return (
                      <React.Fragment key={idx}>
                        <div className="flex flex-col items-center flex-shrink-0">
                          <div className="w-[46px] sm:w-[52px] lg:w-[50px] xl:w-[56px] h-[50px] sm:h-[56px] lg:h-[54px] xl:h-[60px] bg-white rounded-xl shadow-xs border border-white flex flex-col items-center justify-center p-0.5 text-center group hover:scale-105 transition-transform">
                            <Icon className="w-4 h-4 text-[#F4512A] stroke-[2.2] mb-0.5" />
                            <span className="text-[8px] sm:text-[8.5px] font-bold text-[#102126] leading-tight text-center">
                              {node.label}
                            </span>
                          </div>
                        </div>
                        ...
                        {idx < ekhumNodes.length - 1 && (
                          <div className="flex items-center justify-center text-white/60 select-none px-0.5 flex-shrink-0">
                            <span className="text-[10px] sm:text-[11px] font-bold text-white/70">→</span>
                          </div>
                        )}
                      </React.Fragment>
                    );
                ...
                {/* Flowing Orange Wave Connector Line */}
                <div className="relative w-full h-5 sm:h-6 my-1 pointer-events-none hidden sm:block">
                  <svg className="w-full h-full text-[#F4512A]" viewBox="0 0 360 24" fill="none" preserveAspectRatio="none">
                    {/* Node connector drop lines */}
                    ...
                    {/* Smooth undulating wave curve across the nodes */}
                    <path 
                      d="M 18 8 Q 53 14, 88 8 T 148 8 T 212 8 T 272 8 T 342 8" 
                      stroke="currentColor" 
                      strokeWidth="2.2" 
                      strokeLinecap="round" 
                    />
                    
                    {/* Smooth central drop curve to EKhum logo */}
                    <path 
                      d="M 180 8 C 180 14, 180 18, 180 24" 
                      stroke="currentColor" 
                      strokeWidth="2.4" 
                      strokeLinecap="round" 
                    />
                ...
                {/* Sleek Inline EKhum Logo: Aligned horizontally in one row, no tagline, reduced logo size, no white container */}
                <div className="flex items-center justify-center gap-2 mt-1 sm:mt-1.5">
                  <div className="relative w-4.5 h-4.5 sm:w-5 sm:h-5 flex items-center justify-center flex-shrink-0">
                    <svg 
                      viewBox="0 0 36 36" 
                      fill="none" 
                      xmlns="http://www.w3.org/2000/svg" 
                      className="w-full h-full text-[#F4512A] animate-[spin_10s_linear_infinite]"
                    >
                  ...
                  <div className="flex items-center tracking-tight font-extrabold text-lg sm:text-xl leading-none">
