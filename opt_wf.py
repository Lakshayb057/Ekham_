...
# Add RoadmapLoop definition and replace original implementation
if 'RoadmapLoop' not in c:
    loop_code = """
...
"""
    ...
    pattern = re.compile(r'<div className=\"overflow-x-auto pb-1 pt-3 scrollbar-none\">.*?<\/div>\s*<\/div>\s*<\/div>\s*\{\/\* =========================================================================\s*5\. TRUST OR CLOSING LOGO', re.DOTALL)
    match = pattern.search(c)
    if match:
        c = c[:match.start()] + '<RoadmapLoop timelineSteps={timelineSteps} />\n        </div>\n\n        {/* =========================================================================\n            5. TRUST OR CLOSING LOGO' + c[match.end():]
        print('Roadmap replaced!')
    else:
