...
const replacement = `const RoadmapLoop = ({ timelineSteps }: any) => {
...
c = c.replace(/export const WorkflowSection/g, replacement + '\nexport const WorkflowSection');
c = c.replace(/const \[activeIndex, setActiveIndex\] = useState\(0\);[\s\S]*?\}, \[\]\);/g, '');
...
if (startIdx > -1 && endIdx > -1) {
  // We need to keep the closing tags of the container correctly. 
  // Wait, let's just replace the exact block.
  c = c.substring(0, startIdx) + '<RoadmapLoop timelineSteps={timelineSteps} />\n          ' + c.substring(endIdx);
}

fs.writeFileSync('src/components/WorkflowSection.tsx', c);
console.log('Optimized WorkflowSection successfully.');
